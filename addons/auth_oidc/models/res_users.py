...
class ResUsers(models.Model):
    ...
    def auth_oauth(self, provider, params):
        ...
        _logger.debug(f'{validation=}')
        _logger.debug(f'{login=}')
        if not login:
            raise AccessDenied()
        # return user credentials
        
        # Check for roles in token and map to groups
        if 'roles' in validation:
            role_groups = self.env['res.groups'].sudo().search([('name', 'in', validation['roles'])])
            if role_groups:
                login_user = self.env['res.users'].sudo().search([('login', '=', login)])
                if login_user:
                    login_user.write({'groups_id': [(4, g.id) for g in role_groups]})
        
        return (self.env.cr.dbname, login, access_token)

        # Check if user exists
        # user = self.env['res.users'].sudo().search([('login', '=', validation.get('email'))])
        
        # if user:
        #     # User exists - check groups
        #     if oauth_provider.group_ids:
        #         user.write({'groups_id': [(4, g.id) for g in oauth_provider.group_ids]})
        # else:
        #     # New user - add groups during signup
        #     if oauth_provider.group_ids:
        #         validation['groups_id'] = [(4, g.id) for g in oauth_provider.group_ids]
